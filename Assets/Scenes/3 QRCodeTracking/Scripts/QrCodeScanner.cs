...
{
#if ZXING_ENABLED
    [SerializeField] private int sampleFactor = 2; // Checando se intensificar o downsample permite que os dois QR codes sejam detectados mais distantes um do outro
    ...
    public async Task<QrCodeResult[]> ScanFrameAsync()
    {
        ...
        try
        {
            ...
            {
                DebugLogger.LogWarning("Camera helper is not assigned.");
                return null;
            }
            ...
            return await Task.Run(() =>
            {
                ...
                {   if (detectionMode == QrCodeDetectionMode.Single)
                    {
                        ...
                        {
                            DebugLogger.Log($"Not null scanner result. Text of decoded qrcode : {decodeResult.Text}");
                    ...
                    else
                    {
                        ...
                        if (decodeResults != null)
                        {
                            ...
                            DebugLogger.Log($"Not null scanner result. Count of decoded qrcodes : {results.Count}");
                ...
                {
                    DebugLogger.LogError($"Error decoding QR code(s): {ex.Message}");
                }
                return null;
            });
        }
    ...
    private QrCodeResult ProcessDecodeResult(Result decodeResult, int targetWidth, int targetHeight)
    {
        ...
        var uvCorners = new Vector2[points.Length];
        DebugLogger.Log($"ProcessDecodeResult : decodeResult.ResultPoints={decodeResult.ResultPoints}");
        for (var i = 0; i < points.Length; i++)
        {
            uvCorners[i] = new Vector2(points[i].X / targetWidth, points[i].Y / targetHeight);
        }
